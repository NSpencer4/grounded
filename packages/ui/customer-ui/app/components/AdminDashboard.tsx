...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const [selectedConversation, setSelectedConversation] = useState<ConversationWithProfiles | null>(
    null,
  )
  ...
  const handleCreateRep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage((error as Error).message || 'Failed to create representative profile')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleUpdateRep = async (rep: Profile) => {
    try {
      ...
      setMessage((error as Error).message || 'Failed to update representative')
    }
  }
  ...
  const handleDeleteRep = async (rep: Profile) => {
    ...
    try {
      ...
      setMessage((error as Error).message || 'Failed to delete representative')
    }
  }
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage((error as Error).message || 'Failed to create admin profile')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleUpdateAdmin = async (admin: Profile) => {
    try {
      ...
      setMessage((error as Error).message || 'Failed to update admin')
    }
  }
  ...
  const handleDeleteAdmin = async (admin: Profile) => {
    ...
    try {
      ...
      setMessage((error as Error).message || 'Failed to delete admin')
    }
  }
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form
              onSubmit={activeTab === 'representatives' ? handleCreateRep : handleCreateAdmin}
            ...
            >
              ...
              <div className="flex space-x-3 pt-4">
                ...
                >
                  {loading
                    ? 'Creating...'
                    : `Create ${activeTab === 'representatives' ? 'Representative' : 'Admin'} Profile`}
                </button>
                <button
                  type="button"
                  onClick={() => {
            ...
            <div className="sticky top-0 bg-white border-b border-slate-200 p-6 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center space-x-4 text-sm text-slate-600">
                  <span>
                    Customer: <strong>{selectedConversation.customer?.name}</strong>
                  </span>
                  <span>|</span>
                  <span>
                    Representative:{' '}
                    <strong>{selectedConversation.representative?.name || 'Unassigned'}</strong>
                  </span>
                  <span>|</span>
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        <div className={`max-w-[70%] ${isCustomer ? 'order-1' : 'order-2'}`}>
                          <div className="flex items-baseline space-x-2 mb-1">
                            <span className="text-xs font-medium text-slate-600">
                              {isCustomer
                                ...
                                : selectedConversation.representative?.name}
                            </span>
                          ...
                          <div
                            className={`rounded-lg px-4 py-2 ${
                              isCustomer ? 'bg-slate-100 text-slate-900' : 'bg-blue-600 text-white'
                            }`}
                          >
