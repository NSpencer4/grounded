...
interface RepChatViewContentProps {
  userName?: string
}

export default function RepChatViewContent({ userName = 'Agent' }: RepChatViewContentProps) {
      ...
      <aside className="w-80 border-r border-slate-200 flex flex-col bg-white">
        <div className="p-4 flex flex-col gap-4 border-b border-slate-200">
          <div className="flex items-center gap-2">
            <div className="flex flex-1 items-stretch rounded-lg h-10">
              ...
              <input
                ...
                className="flex w-full min-w-0 flex-1 border-none bg-slate-100 focus:ring-0 h-full placeholder:text-slate-500 px-3 rounded-r-lg text-sm font-normal outline-none"
                placeholder="Search chats..."
              />
            </div>
            <button className="flex items-center justify-center size-10 rounded-lg bg-blue-600 text-white hover:bg-blue-700">
      ...
      <section className="flex-1 flex flex-col bg-slate-50 relative min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          {messages.map((message) => {
            ...
            if (message.type === 'ai') {
              return (
                <div key={message.id} className="flex gap-3 max-w-[80%] self-end flex-row-reverse">
