...
import { createGraphQLClient, LIST_BUDGETS, type Budget } from '../lib/graphql'
...
export async function loader({ context }: LoaderFunctionArgs) {
  ...
  try {
    // Fetch budgets
    const budgetData = await graphqlClient.request<{ budgets: Budget[] }>(LIST_BUDGETS, {
      orgId,
      limit: 1,
    })

    // Fetch performance metrics from our API route
    const metricsResponse = await fetch(
      `http://localhost:5173/api/performance?orgId=${orgId}`,
    ).catch(() => null)
    const metricsData = metricsResponse ? await metricsResponse.json() : null

    // Fetch escalations from our API route
    const escalationsResponse = await fetch(
      `http://localhost:5173/api/tickets?type=escalations&orgId=${orgId}&limit=5`,
    ).catch(() => null)
    const escalationsData = escalationsResponse ? await escalationsResponse.json() : null

    // Transform data for display
    const metrics = metricsData?.metrics
      ? {
          activeChats: metrics.conversations?.active || 142,
          avgResponseTime: `${Math.floor((metrics.representatives?.averageResponseTime || 84) / 60)}m`,
          escalationRate: `${((metrics.conversations?.escalated / metrics.conversations?.total) * 100 || 8.2).toFixed(1)}%`,
          aiResolutionRate: `${((metrics.ai?.successRate || 0.64) * 100).toFixed(0)}%`,
        }
      : {
          activeChats: 142,
    ...
    return json<DashboardData>({
      ...
      escalations: escalationsData?.escalations?.slice(0, 3) || [],
    })
  } catch (error) {
...
export default function DashboardRoute() {
  ...
  const statsCards = [
    {
      label: 'Active Chats',
      value: data.metrics.activeChats.toString(),
      icon: MessageCircle,
    ...
    },
    {
      ...
      value: data.metrics.aiResolutionRate,
      icon: Zap,
  ...
  const budgetUsedPercent = budget ? (budget.spent / budget.amount) * 100 : 42
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-slate-900">
                        ${budget ? budget.spent.toFixed(2) : '4,200.00'}
                      </p>
                    </div>
                    <p className="text-slate-500 text-sm">
                      of ${budget ? budget.amount.toFixed(0) : '10,000'}
                    </p>
                  </div>
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed italic">
                    {budgetUsedPercent.toFixed(0)}% of monthly budget consumed
                  </p>
                </div>
              </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="size-8 rounded-full bg-slate-200 flex items-center justify-center font-bold text-xs text-slate-600">
                              {escalation.customerName?.charAt(0) || 'U'}
                            </div>
                            <div>
                              <p className="text-sm font-semibold text-slate-900">
                                {escalation.customerName || 'Unknown'}
                              </p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm max-w-xs truncate text-slate-700">
                          {escalation.issue || escalation.reason || 'No description'}
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-2 py-1 rounded text-[10px] font-bold uppercase bg-orange-100 text-orange-600">
                            {escalation.status}
                          </span>
                        </td>
